...
void draw_sprite(Tsprite * spr) {
    ...
    blit_ex(&buffer, &sprite->m_image, spr->x - (sprite->w >> 1), spr->y  - (sprite->h >> 1), sprite->u, sprite->v, sprite->w, sprite->h, spr->flags);
}
...
void draw_tile16_fast(Tsprite * spr) {
    ...
    blit_ex(&buffer, &sprite->m_image, spr->x, spr->y, sprite->u, sprite->v, sprite->w, sprite->h, spr->flags);
}
